---
layout: post
title:  "Training agents for multiplayer games with Stable Baselines"
date: "2025-10-08"
...
This is part 1 in my blog series on reinforcement learning lessons from my progress on Photosynthesis AI. A basic knowledge of reinforcement learning will be assumed, but I will aim to clarify anything that is a bit more unusual. More details on the algorithm, the project, the custom environment etc, can be found on the project page **LINK**. The purpose of this series is to do a bit of a deeper dive into a few design decisions. Some of the ideas here will hopefully be quite instructive for anyone else wanting to build a reinforcement learning algorithm.
...
Gymnasium (a fork of the now unmaintained Gym) is one of the most popular libraries for defining environments for training reinforcement learning models. It is often used in conjunction with the library Stable Baselines 3, which defines the models and deals with the training procedure (propogating rewards back through episodes, applying temporal discounting, and so on). The basic code for training an agent is very simple. 
...
An alternative is to train offline. To do this we can use a set of _experience buffers_, as show in the image below.
...
This can work fine for off-policy agents such as ACER. However, I wanted to use PPO, which is an on-policy method, and so cannot (at least in SB3) learn from an external buffer.

## Solution with masked environment

The solution that I have used with Photosynthesis AI, that is generally applicable to similar multiplayer games, is to use a _masked environment_.

Here, we define multiple agents. One agent is the _active agent_, that will undergo on-policy learning with all states that it receives.

The other agents will be _frozen agents_. These will be subsumed along with the environment into a larger _meta-environment_, such that the meta-environment only returns states from the perspective of the active player. See the below image.

<figure>
<img src="/assets/images/posts/multiplayer/cards_rl_3.png"/>
</figure>

#TODO Explain the image in detail.

Here, the active agent sends 
        ...
        def step(self, action : int):
            ...
            while True:
                ...
                if self.agents[self.state_h.active_player] is None:
                    ...
                    valid_actions = self.get_action_mask(self)
...
This is then instantiated at training stage like this:
...
After some condition is reached, either a predefined number of training timesteps, number of episodes, or performance level of the active agent, we take the saved weights of the active agent, freeze then, instantiate three frozen agents using these weights, and continue the training process, as shown in the below image.
...
This results in a non-smooth and possible discontinuous reward curve. Below is shown an example of this. Here, we see the win-rate of the active agent in blue, and the reward of the active agent in orange. The reward function being used here is $S$ when the active agent achieves a score of $S$ and come in first place, and 0 if they do not come in first place.

As expected for a four-player game, the win-rate starts at 25%, and soon rises. The vertical dashed lines show the points where the active model weights are distribuited to the frozen players.
